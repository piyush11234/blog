...
export default function CommentBox({ selectedBlog }) {
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:8080/api/comment/${selectedBlog._id}/create`,
        { content: trimmed },
  ...
  const handleEdit = async (commentId) => {
    ...
    try {
      const res = await axios.put(
        `http://localhost:8080/api/comment/${commentId}/edit`,
        { content: trimmed },
        { headers: { "Content-Type": "application/json" }, withCredentials: true }
      );
  ...
  const handleDelete = async (commentId) => {
    try {
      const res = await axios.delete(`http://localhost:8080/api/comment/${commentId}/delete`, {
        withCredentials: true
      });
  ...
  const fetchAllComments = async () => {
    try {
      const res = await axios.get(`http://localhost:8080/api/comment/${selectedBlog._id}/comment/all`);
